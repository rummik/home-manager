...
let
  ...
  mkOptionString = prefix: option:
    removeSuffix "__" "${prefix}${camelToSnake option}";

  attrsToKittyConfig = prefix: attrs:
    concatStringsSep "\n" (
      mapAttrsToList (option: value: "${mkOptionString prefix option} ${mkValueString value}") attrs
    );

  listToKittyConfig = prefix: list:
    concatStringsSep "\n" (
...

{
  ...
  options.programs.kitty = {
    ...
    fonts = {
      ...
      symbolMap = mkOption {
        ...
        default = {};
        description = "Map symbols from font";
      };
    };
  ...
  config = mkIf cfg.enable {
    ...
    xdg.configFile."kitty/kitty.conf".text = /* conf */ ''
      ${attrsToKittyConfig "" (removeAttrs cfg.fonts [ "symbolMap" ])}
